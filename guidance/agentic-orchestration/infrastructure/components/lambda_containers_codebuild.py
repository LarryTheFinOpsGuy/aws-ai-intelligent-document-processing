...
class LambdaContainersCodeBuildConstruct(Construct):
    def __init__(self, scope: Construct, construct_id: str, config: dict, core_stack, **kwargs) -> None:
        ...
        self._create_codebuild_project("S3VectorBuild", self.s3_vector_repo, "tools/s3_vector_tool", codebuild_role)
        self._create_codebuild_project("TextractorBuild", self.textractor_repo, "tools/textractor_tool", codebuild_role)
        self._create_codebuild_project("S3BucketBuild", self.s3_bucket_repo, "tools/s3_bucket_tool", codebuild_role)
        ...
        # Create Lambda functions using ECR images
